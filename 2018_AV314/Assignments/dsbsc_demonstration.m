...
received_dsbsignal_ooft = conv(dsbsignal_ioft,channel_hoft);
plotspectrum(received_dsbsignal_ooft);
pause;
...
tr = 0:(1/fs):(length(received_dsbsignal_ooft)/fs - 1/fs);
%% DSB (DSBSC) demodulation - Generate carrier at the receiver local oscillator
A = 1;
fc = 3000;
phase = pi/4;
...
multiplied_output = rx_carrier .* received_dsbsignal_ooft;
